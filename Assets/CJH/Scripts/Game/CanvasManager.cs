...
public class CanvasManager : MonoBehaviourPun//, IPunObservable
{
    ...
    private void Awake()
    {
        ...
        for (int i = 0; i < puz.transform.childCount; i++)
            puzzle[i] = puz.transform.GetChild(i).gameObject;         //각 퍼즐들의 정보
    }
    ...
    public void SetPuzzlePosition()                                //11 * 11 판에 퍼즐의 좌표 지정하는 함수
    {
        if (PhotonNetwork.IsMasterClient)           //방장일 때 생성
        {
            int k = 0;
            while (k < puzzle.Length)
            {
                ...
                photonView.RPC("SetQuadColor", RpcTarget.AllBuffered, x, y, k, index);   //퍼즐 틀 위치 전해주기
                k++;
            }
        }
    }
