...
Shader "Space Graphics Toolkit/Planet"
{
   Properties
   {
	...
	[NoScaleOffset][Normal]_DetailMapR_Ext_1("	Normal R", 2D) = "bump" {}
	[NoScaleOffset][Normal]_DetailMapG_Ext_1("	Normal G", 2D) = "bump" {}
	[NoScaleOffset][Normal]_DetailMapB_Ext_1("	Normal B", 2D) = "bump" {}
	[NoScaleOffset][Normal]_DetailMapA_Ext_1("	Normal A", 2D) = "bump" {}
...
#   define UNITY_LOOP
	...
	float4 sample_spherical(sampler2D s, float4 uv, float cur_height, int max_height)
	{
		...
		//float4 uvA     = uv + sin(float4(89.0f, 97.0f, 89.0f, 97.0f) / 15.113f * this_index)*0;
		//float4 uvB     = uv + sin(float4(89.0f, 97.0f, 89.0f, 97.0f) / 15.113f * next_index)*0;
		float4 uvA     = uv + sin(float4(1.0f, 2.0f, 1.0f, 2.0f) * this_index)*0.5f;
		float4 uvB     = uv + sin(float4(1.0f, 2.0f, 1.0f, 2.0f) * next_index)*0.5f;
		float4 gradX   = ddx(uv);
		float4 gradY   = ddy(uv);
	...
	sampler2D _DetailMapR_Ext_1;
	sampler2D _DetailMapG_Ext_1;
	sampler2D _DetailMapB_Ext_1;
	sampler2D _DetailMapA_Ext_1;
	...
	void Ext_SurfaceFunction5(inout Surface o, inout ShaderData d)
	{
		...
		#if _DETAIL_R
		ContributeDetail(o, _DetailMapR_Ext_1, coords, polar, max_height, noise, d.blackboard.detailMask.r * _DetailScaleR_Ext_1 * fade);
		#endif
		#if _DETAIL_G
		ContributeDetail(o, _DetailMapG_Ext_1, coords, polar, max_height, noise, d.blackboard.detailMask.g * _DetailScaleG_Ext_1 * fade);
		#endif
		#if _DETAIL_B
		ContributeDetail(o, _DetailMapB_Ext_1, coords, polar, max_height, noise, d.blackboard.detailMask.b * _DetailScaleB_Ext_1 * fade);
		#endif
		#if _DETAIL_A
		ContributeDetail(o, _DetailMapA_Ext_1, coords, polar, max_height, noise, d.blackboard.detailMask.a * _DetailScaleA_Ext_1 * fade);
		#endif
	#endif
	}
